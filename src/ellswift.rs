...
use ffi::secp256k1_ellswift_xdh;

use crate::ffi::{
    self, secp256k1_ellswift_create, secp256k1_ellswift_decode, secp256k1_ellswift_encode, CPtr,
};
use crate::{constants, from_hex, Context, PublicKey, Secp256k1, SecretKey};
...
/// This object holds tow field elements u and t, which are the inputs to
...
impl ElligatorSwift {
    ...
    /// Computes the ElligatorSwift encoding for a valid public key, using some
    ...
    /// let rnd = [1u8; 32];
    /// let es = ElligatorSwift::from_pubkey(pk, rnd);
    /// ```
    pub fn from_pubkey(pk: PublicKey, rand32: [u8; 32]) -> ElligatorSwift {
        Self::ellswift_encode(pk, rand32)
    }
    ...
    pub fn shared_secret(
        ...
        unsafe {
            secp256k1_ellswift_xdh(
                ...
                data.map(|d| d.as_ptr()).unwrap_or(std::ptr::null()) as *mut c_void,
            );
        }
        ElligatorSwiftSharedSecret(shared_secret)
    }
    /// Constructs an ElligatorSwift object from a 64-byte hex string.
    /// # Example
    /// ```
    /// use secp256k1::ellswift::ElligatorSwift;
    /// let es = ElligatorSwift::from_hex("c01204f76d2f886a4d3319e2ac0c86a2cb560ec29c732336a99ff152");
    /// ```
    pub fn from_hex(hex: &str) -> ElligatorSwift {
        let mut ser = [0u8; 64];
    ...
    fn ellswift_encode(pk: PublicKey, rand32: [u8; 32]) -> ElligatorSwift {
        let mut ell_out = [0u8; constants::ELLSWIFT_ENCODING_SIZE];
        unsafe {
...
impl Deref for ElligatorSwiftSharedSecret {
    type Target = [u8; 32];
...
impl From<ElligatorSwiftParty> for i32 {
    fn from(party: ElligatorSwiftParty) -> Self {
        match party {
...
impl CPtr for ElligatorSwift {
    type Target = ffi::ElligatorSwift;
...
mod tests {
    ...
    use crate::{
        ellswift::{ElligatorSwift, ElligatorSwiftParty, ElligatorSwiftSharedSecret},
        from_hex, PublicKey, SecretKey, XOnlyPublicKey,
    };
    #[test]
    fn test_elligator_swift_rtt() {
        ...
        let ell = ElligatorSwift::from_pubkey(public_key, [1u8; 32]);
